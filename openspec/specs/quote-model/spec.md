...
## Purpose
Persistence and invariants of quotes: gapless yearly numbering, full-row versions under a shared number, lines with per-line discount and VAT rounded half up, freezing on send, acceptance that wins the opportunity, stored PDFs, the mail outbox and application settings.
