...
## Purpose
Mobile-first quote screens: filterable list with the expiry visual, quote sheet with the version chain and email status, draft form with live totals, send dialog with the interpolated template, accept/reject dialogs and the admin defaults screen.
