...
Imports (`POST /api/v1/products/import`, `POST /api/v1/accounts/import`; `admin` and `back_office` only): multipart `.csv`/`.xlsx` upload with `dry_run=true` by default — the response is the full per-row report (`created` / `updated` / `unchanged` / `error` with the reason); confirming re-posts with `dry_run=false` and applies valid rows through the same services as manual API calls (row-by-row commits, idempotent matching makes re-runs safe). Caps: 2 000 data rows / 5 MB. Matching: products by normalised SKU (the change-05 `upsert_by_sku` contract; brand/family resolve by name, never created), accounts by normalised CIF with exact-normalised-name fallback (never fuzzy, never renamed, back-office updates limited to administrative fields), embedded contacts by email within the account. Header aliases are case- and accent-insensitive; the canonical Spanish columns are — catalogue: `Código` (SKU), `Nombre`, `Marca`, `Familia`, `Tipo` (equipo/consumible/servicio), `PVP`, `Coste`, `Unidad`, `Descripción`; accounts: `Nombre`, `CIF`, `Provincia` (código INE), `Ciudad`, `Dirección`, `Código postal`, `Teléfono`, `Email`, `Tipo`, plus optional `Contacto nombre`, `Contacto apellidos`, `Contacto email`, `Contacto teléfono`, `Cargo`, `Especialidad` (repeat the account columns to add several contacts; an unknown specialty is a message on the row, not an error). Prices accept Spanish decimal commas; CSV accepts `;` or `,` in UTF-8 or cp1252; `.xlsx` reads the first worksheet. Each confirmed run records `import.products_executed` / `import.accounts_executed` with the counts; dry runs record nothing. Frontend routes: `/buscar`, `/importar/catalogo`, `/importar/centros` (cards in Admin and Más).

Phones (change 12): centres and contacts hold an ordered list of labelled phones instead of single columns. The label is free text (the UI suggests Principal, Secretaría, Servicio, Consulta, Despacho, Extensión, Móvil, Fax) and the **extension is its own field**, so the number stays E.164 (`+34…`) and the UI renders `tel:+34915550001;ext=4021`; a number carrying "ext" text is rejected with `phone_invalid`. The first entry is the primary one — that is what lists, cards and importers read, and "Hacer principal" simply moves a row to the front. Global search matches the digits of **any** phone in either list, so a centre is found by its switchboard or by its vascular-service number. The importers keep their `Teléfono` / `Contacto teléfono` columns: they create or update the primary phone and leave the other labelled numbers alone. Contact phones are personal data and anonymisation deletes them; centre phones are not touched. Accounts also gained `billing_notes` (free-text invoicing and accounting contact, editable by back office) and contacts an independent "Jefe de servicio" tick — a doctor can be a vascular surgeon *and* head of department.

Activity calendar (`GET /api/v1/activities/calendar?year=&month=&owner_id=`): one unpaginated request returns a month of compact entries (id, Madrid-local `occurred_on`/`occurred_time`, status, type, centre, owner), capped at 1000 with the uncapped `total`. An activity appears on the day of its timeline `occurred_at` (`done_at` when done, `scheduled_at` otherwise); cancelled activities are excluded. Staff (`admin`, `sales_manager`, `back_office`) see the whole team and may filter one rep with `owner_id`; a `sales_rep` always gets their own month and requesting a colleague's is a 403. Frontend: the Hoy page's Día ↔ Mes switcher.

Specialties and the contacts list (change 13): a contact carries a **medical specialty**
(`specialty_id`, what the person practises) instead of a commercial division (how Quermed
organises its own product lines) — `GET /api/v1/specialties` and the reference bundle serve
the twelve-entry catalogue. `GET /api/v1/contacts` lists contacts across every account the
caller can see, introducing no new permission concept: the account visibility rule is applied
through the contact. Its filters are **cumulative** — repeating `specialty_id` or `account_id`
adds up (OR), different filters narrow (AND) — and it also accepts `q` (accent-insensitive
over first name, last name and full name), `job_title_id`, `is_head_of_department` and
`is_active`; an unknown filter value returns an empty page, never an error. Names sort
